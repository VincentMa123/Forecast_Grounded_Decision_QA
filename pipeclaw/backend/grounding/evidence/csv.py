...
def build_csv_evidence(
    ...
    *,
    ...
    usable_outputs = []
    for item in attach_tool_arguments(tool_outputs, tool_calls):
        output = item.get("output") or {}
        if tool_evidence_unavailable(item) or not isinstance(output, dict):
            continue
        usable_outputs.append((str(item.get("name") or "").casefold(), item, output))
    answer_text, folded_scope = answer.casefold(), scope_text.casefold()
    answer_numbers, scope_numbers = (
        {_normalized_number(value) for value in NUMBER_RE.findall(text)}
        for text in (answer, scope_text)
    )
    ...
    for name, item, output in usable_outputs:
        ...
        for row in rows:
            ...
            score = (_row_relevance(row, answer_text, answer_numbers) * 2
                     + _row_relevance(row, folded_scope, scope_numbers))
            if score:
                candidates.append((score, len(seen_rows) - 1, source_file, row))

    for source_file in _dynamic_script_csv_files(usable_outputs, answer, scope_text):
        remember_source(source_file)

    for name, _, output in usable_outputs:
        if name != "run_command":
            continue
        for source_file in map(lambda match: Path(match).name,
                               CSV_FILE_RE.findall(str(output.get("stdout") or ""))):
            if scoped_dates and not any(date in source_file for date in scoped_dates):
                continue
            remember_source(source_file)

    computed_results = _structured_computation_results(
        usable_outputs, answer_text, answer_numbers)
    ...
    if computed_results:
        ...
        evidence["answer_rows"] = [
            {"source_file": source_file, "values": _compact_row(row)}
            for _, _, source_file, row in selected
        ]
        selected_rows = [(source_file, row) for _, _, source_file, row in selected]
        evidence["selection_summary"] = _selection_summary(selected_rows)
        aggregation_rows = _named_scope_aggregation_rows(
            scoped_rows, selected_rows, combined_text=scope_text)
        derived_results = _derived_aggregate_results(
            aggregation_rows, answer_text=answer_text,
            answer_numbers=answer_numbers, scope_text=folded_scope)
        if derived_results:
...
def _dynamic_script_csv_files(
    outputs: Iterable[tuple[str, Dict[str, Any], Dict[str, Any]]],
    answer: str,
    scope_text: str,
) -> List[str]:
...
def _selection_summary(
    ...
    def endpoint(row: Mapping[str, Any], columns: Iterable[str]) -> str:
        return next((str(row.get(column) or "").strip() for column in columns
                     if str(row.get(column) or "").strip()), "")
    segment_pairs = {
        (endpoint(row, START_COLUMNS), endpoint(row, END_COLUMNS))
        for _, row in selected_rows
        if endpoint(row, START_COLUMNS) and endpoint(row, END_COLUMNS)
    }
    summary = {
        "selected_row_count": len(selected_rows),
        "selected_source_file_count": len({source for source, _ in selected_rows}),
    }
    if pipelines:
...
def _derived_aggregate_results(
    ...
    *,
    ...
    """Verify aggregate claims from bounded, query-scoped CSV rows."""
    combined_text = f"{scope_text}\n{answer_text}"
    if not AGGREGATION_RE.search(combined_text):
        return []

    group_column, measure_column = _aggregate_columns(selected_rows, combined_text)
    ...
    for source_file, row in selected_rows:
        group_value = str(row.get(group_column) or "").strip() if group_column else ""
        number = _decimal_value(row.get(measure_column))
        if number is None or (group_column and not group_value):
            continue
    ...
    for group_value, values in grouped.items():
        ...
        if group_column:
            ...
            nonzero_count = sum(value != 0 for _, value in values)
            zero_count = len(values) - nonzero_count
            percentages = (
                ("nonzero_row_percentage", nonzero_count),
                ("zero_row_percentage", zero_count),
            )
            result = {
                "operation": operation,
                "measure": measure_column,
                ...
                **{
                    key: _decimal_to_number(value)
                    for key, count in percentages
                    if _decimal_is_claimed(
                        value := Decimal(count) * Decimal("100") / Decimal(len(values)),
                        answer_numbers,
                    )
                },
                "source_files": source_files,
...
def _aggregate_columns(
    ...
    measure_column = max(candidates, key=lambda item: item[0])[1] if candidates else ""
    return group_column, measure_column
...
def _structured_computation_results(
    ...
    """
    for name, item, output in reversed(list(outputs)):
        if name != "run_command":
            continue
        ...
        try:
            ...
            value = stdout
        if not _computation_supports_answer(value, answer_text, answer_numbers):
            continue
        compact_value = (_compact_json_value(value) if isinstance(value, (dict, list))
                         else stdout[:MAX_COMPUTED_OUTPUT_CHARS])
        return [
            {
...
def _computation_supports_answer(
    value: Any,
    answer_text: str,
    answer_numbers: set[str],
) -> bool:
    if isinstance(value, (dict, list)):
        ...
        return False
    text = str(value)
    output_numbers = {_normalized_number(item) for item in NUMBER_RE.findall(text)}
