...
def _status_summary(category_status: Dict[str, Any], chinese: bool) -> str:
    ...
    return "; ".join(
        f"{'/'.join(values)}={status}" for status, values in groups.items()
    )
...
def _finding_summary(findings: List[Dict[str, Any]], chinese: bool) -> str:
    ...
    for finding in findings[:2]:
        ...
        if values:
            ...
            detail = f"; {metric}={format_number(evaluated.get('value'))}"
...
def _watch_summary(output: Dict[str, Any]) -> List[str]:
    ...
    if not watch:
        ...
        watch = [
            {"variable": variable, **dict(summary or {})}
            for variable, summary in list(summaries.items())[:3]
        ]
    result = []
    for item in watch[:3]:
        ...
        result.append(
            f"{variable}={format_number(value)}" if value is not None else variable
        )
...
def _single_forecast_answer(question: str, tool_result: Dict[str, Any]) -> str:
    ...
    linepack = dict(
        dict(verification.get("engineering_evidence") or {}).get("linepack") or {}
    )
    ...
    if _LINEPACK_DECLINE_REQUEST.search(question):
        if linepack_status == "pass" and (
            decline_minutes is None or float(decline_minutes) <= 0
        ):
            lines.append(
                "持续管存下降未在当前预测中出现。"
                if chinese
                else "Sustained linepack decline did not appear in the current forecast."
            )
        elif linepack_status in {"warning", "fail"} or (
            decline_minutes is not None and float(decline_minutes) > 0
        ):
            duration = (
                format_number(decline_minutes)
                if decline_minutes is not None
                else "unknown"
            )
            lines.append(
                f"预测检测到管存下降，最长连续下降 {duration} 分钟。"
                if chinese
                else f"The forecast detected linepack decline; maximum continuous duration was {duration} minutes."
            )
    if impact_count is not None:
        lines.append(
            f"基线对比检出 {int(impact_count)} 个变化输出变量。"
            if chinese
            else f"Baseline comparison found {int(impact_count)} changed output variables."
        )
    ...
    lines.append(
        f"风险 {risk}；人工干预 {intervention}。"
        if chinese
        else f"Risk: {risk}; intervention: {intervention}."
    )
    findings = [dict(item) for item in verification.get("priority_findings") or []]
    if findings:
        summary = _finding_summary(findings, chinese)
        lines.append(
            f"优先发现：{summary}。" if chinese else f"Priority findings: {summary}."
        )
    ...
    if watch:
        lines.append(
            f"关注变量：{', '.join(watch)}。"
            if chinese
            else f"Watch variables: {', '.join(watch)}."
        )
...
def _compact_answer(answer: str, maximum_chars: int) -> str:
    ...
    sentences = [
        part.strip()
        for part in re.split(r"(?<=[。！？!?])\s*|(?<=[A-Za-z])\.\s+", compact)
        if part.strip()
    ]
    if not sentences:
...
def repair_grounded_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    legacy_method = str(
        dict(repaired.get("repair_provenance") or {}).get("method") or ""
    )
    ...
    if contract.get("answer_mode") == "dispatch_comparison":
        ...
        repaired["dispatch_recommendation"] = str(
            decision.get("selected_dispatch_recommendation") or ""
        )
    ...
    should_render_single = bool(pipeformer_results) and (
    ...
    if contract.get("answer_mode") != "dispatch_comparison" and should_render_single:
        ...
        repaired["risk_level"] = verification.get("risk_level") or output.get(
            "risk_level"
        )
        repaired["manual_intervention_label"] = verification.get(
            "human_intervention_label"
        ) or output.get("manual_intervention_label")
        repaired["dispatch_recommendation"] = str(
            verification.get("dispatch_recommendation") or ""
        )
        repaired["repair_provenance"] = {
            "method": "scenario_aware_deterministic_repair",
            "external_llm_calls": 0,
            "reason": "Single-forecast answer rebuilt from stored tool evidence.",
        }
    if "unsupported_unit_claim" in issues:
        ...
        if stripped != repaired.get("final_answer"):
            ...
            repaired["repair_provenance"] = {
                "method": "unsupported_unit_removal",
                "external_llm_calls": 0,
                "reason": "Unsupported unit labels removed without changing the numeric claims.",
            }
    ...
    if "answer_too_long" in issues:
        ...
        compacted = _compact_answer(
            str(repaired.get("final_answer") or ""), maximum_chars
        )
        if compacted != repaired.get("final_answer"):
            repaired["final_answer"] = compacted
            repaired["repair_provenance"] = {
                "method": "extractive_answer_compaction",
                "external_llm_calls": 0,
                "reason": "Formatting and redundant text removed; retained content is extractive.",
            }
    return repaired
