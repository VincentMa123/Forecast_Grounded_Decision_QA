...
class RuleLibrary:
    """Load and cache one JSON-backed engineering rule library."""

    def __init__(self, root: Path = RULE_LIBRARY_ROOT) -> None:
        self.root = Path(root)
    ...
    def manifest(self) -> Dict[str, Any]:
        document = self.load_document("pipeline_constraints.json")
    ...
    def constraint_specs(self, category: str) -> Tuple[ConstraintSpec, ...]:
        rules = self.rule_document(category).get("rules", [])
        specs = []
        seen = set()
        for rule in rules:
    ...
    def rule_definition(self, category: str, rule_id: str) -> Dict[str, Any]:
        for rule in self.rule_document(category).get("rules", []):
            if rule.get("rule_id") == rule_id:
                return dict(rule)
        raise KeyError(f"Rule {rule_id!r} is not defined for category {category!r}")


DEFAULT_RULE_LIBRARY = RuleLibrary()


def load_json_document(filename: str) -> Dict[str, Any]:
    return DEFAULT_RULE_LIBRARY.load_document(filename)


def load_pipeline_constraints() -> Dict[str, Any]:
    return DEFAULT_RULE_LIBRARY.manifest()


def load_rule_document(category: str) -> Dict[str, Any]:
    return DEFAULT_RULE_LIBRARY.rule_document(category)


def load_constraint_specs(category: str) -> Tuple[ConstraintSpec, ...]:
    return DEFAULT_RULE_LIBRARY.constraint_specs(category)


def load_rule_definition(category: str, rule_id: str) -> Dict[str, Any]:
    return DEFAULT_RULE_LIBRARY.rule_definition(category, rule_id)
