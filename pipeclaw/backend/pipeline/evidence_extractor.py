...
def summarize_variables(real_rows: List[ForecastRow], predict_rows: List[ForecastRow]) -> Dict[str, Dict[str, Any]]:
    ...
    for variable in variables:
        ...
        step_changes = [predicted[index] - predicted[index - 1] for index in range(1, len(predicted))]
        minimum = min(predicted) if predicted else None
        maximum = max(predicted) if predicted else None
        peak_index = max(range(len(predicted)), key=lambda index: abs(predicted[index])) if predicted else None
        change_peak_index = (
            max(range(len(step_changes)), key=lambda index: abs(step_changes[index])) + 1
            if step_changes
            else None
        )
        decline_indices = [
        ...
        summaries[variable] = {
            ...
            "minimum_step_index": predicted.index(minimum) if minimum is not None else None,
            "maximum_prediction": round(maximum, 6) if maximum is not None else None,
            "maximum_step_index": predicted.index(maximum) if maximum is not None else None,
            "peak_value": round(predicted[peak_index], 6) if peak_index is not None else None,
...
def top_variables(
    ...
    priority_rank = {variable: len(priority) - index for index, variable in enumerate(priority)}
    ranked = sorted(
        summaries.items(),
        key=lambda item: (
