...
def _project_sft_record(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Project one eligible teacher record into its compact SFT representation."""
    if (
    ...
    projected = {
        key: deepcopy(item[key]) for key in SFT_FIELDS if key in item
    }
...
def write_split_records(
    ...
    for item in records:
        projected = _project_sft_record(item)
        split = item.get("split")
        if projected is not None and split in split_records:
            split_records[split].append(projected)
    written_count = 0
...
__all__ = ["_project_sft_record", "prior_tool_call_provenance", "write_split_records"]
