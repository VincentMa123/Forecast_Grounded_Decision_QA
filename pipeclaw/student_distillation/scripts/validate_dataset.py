...
EXPECTED_SOURCE_COUNTS = {"train": 1073, "valid": 147, "test": 139}
DEFAULT_SOURCE_ROOT = REPO_ROOT / "pipeclaw" / "backend" / "generated_teacher_traces" / "splits"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "pipeclaw" / "student_distillation" / "data"
DEFAULT_MANIFEST_PATH = DEFAULT_OUTPUT_ROOT / "manifests" / "task2_dataset_manifest.json"
_IDENTITY_FIELDS = ("scenario_id", "session_id", "turn_id", "scenario_type")
...
def _normalize_tool_schemas(
    ...
    """Sort and validate registered schemas before release serialization."""
    schemas = sorted(
...
def load_registered_tool_schemas(repo_root: Path = REPO_ROOT) -> list[dict[str, Any]]:
    ...
    schemas, _ = _normalize_tool_schemas(tool_registry.openai_tools_schema())
    return json.loads(stable_json(schemas))
...
def validate_source_records(
    ...
    *,
    ...
    for index, record in enumerate(records, start=1):
        ...
        if missing:
            raise DatasetValidationError(f"{split}:{index}: missing source fields {sorted(missing)}")
        ...
        for field in (*_IDENTITY_FIELDS[:2], "scenario_type", "user_input"):
    ...
    *,
    ...
    for index, record in enumerate(records, start=1):
        ...
        if example_id in seen_example_ids:
            raise DatasetValidationError(f"{projection}/{split}: duplicate example_id {example_id}")
        seen_example_ids.add(example_id)
        _required_text(record, "source_sample_id", example_id)
        for field in (*_IDENTITY_FIELDS[:2], "scenario_type"):
...
def validate_release(
    *,
    ...
    for split in SPLITS:
        ...
        source_by_split[split] = {str(record["sample_id"]): record for record in records}
        for sample_id in source_by_split[split]:
            prior_split = all_source_ids.get(sample_id)
            if prior_split is not None:
                raise DatasetValidationError(f"source split leakage for {sample_id}: {prior_split} and {split}")
        ...
        if source_details.get("record_count") != len(records):
            raise DatasetValidationError(f"{split}: source manifest record count mismatch")
    ...
    for projection in PROJECTIONS:
        ...
        if not isinstance(projection_manifest, dict):
            raise DatasetValidationError(f"{projection}: projection manifest is missing")
        for split in SPLITS:
            expected_relative = f"{projection}/{split}.jsonl"
            location = f"{projection}/{split}"
            details = _manifest_entry(projection_manifest, split, expected_relative, location)
            ...
            if projection == "trace_level":
                observed_tool_schema_hashes.update(_sha256_bytes(str(record["tools"]).encode()) for record in records)
            if details.get("record_count") != len(records):
                raise DatasetValidationError(f"{projection}/{split}: manifest record count mismatch")
            if details.get("sha256") != _sha256_file(output_path):
                raise DatasetValidationError(f"{projection}/{split}: output checksum mismatch")
            ...
            if projection == "constraint_multitask":
                ...
                for task_type in (str(record["task_type"]) for record in records):
                    task_counts[task_type] = task_counts.get(task_type, 0) + 1
                if details.get("task_counts") != dict(sorted(task_counts.items())):
                    raise DatasetValidationError(f"{projection}/{split}: task counts mismatch")
            validated_files += 1
    ...
    for split in CORRECTION_SPLITS if correction_manifest is not None else ():
        expected_relative = f"python_correction/{split}.jsonl"
        location = f"python_correction/{split}"
        details = _manifest_entry(correction_manifest, split, expected_relative, location)
        ...
        if details.get("record_count") != len(records):
            raise DatasetValidationError(f"python_correction/{split}: record count mismatch")
        if details.get("python_record_count") != sum(projection_writes_python(record) for record in records):
            raise DatasetValidationError(f"python_correction/{split}: Python record count mismatch")
        if details.get("sha256") != _sha256_file(path):
            raise DatasetValidationError(f"python_correction/{split}: checksum mismatch")
        source_ids = source_by_split[split]
        if any(record.get("source_sample_id") not in source_ids for record in records):
            raise DatasetValidationError(f"python_correction/{split}: unknown source record")
        validated_files += 1
    return {
        "validated_projection_files": validated_files,
        "validated_source_records": sum(len(records) for records in source_by_split.values()),
    }


def _manifest_entry(
    section: dict[str, Any], key: str, expected: str, location: str
) -> dict[str, Any]:
    details = section.get(key)
    if not isinstance(details, dict):
        raise DatasetValidationError(f"{location}: manifest entry is missing")
    if details.get("file") != expected:
        raise DatasetValidationError(f"{location}: unsafe or unexpected output path")
    return details


def _required_tool_arguments(tool_schemas: Iterable[dict[str, Any]] | None) -> dict[str, set[str]]:
    ...
    for schema in tool_schemas or ():
        ...
        if (
            ...
            and isinstance((required := parameters.get("required")), list)
        ):
            required_by_name[name] = {str(field) for field in required if isinstance(field, str)}
...
def _validate_source_tool_pairs(
    ...
    *,
    ...
    if not isinstance(calls, list) or not isinstance(outputs, list):
        raise DatasetValidationError(f"{sample_id}: tool_calls and tool_outputs must be lists")
    ...
    for call in calls:
        ...
        if not isinstance(call.get("arguments"), dict):
            raise DatasetValidationError(f"{sample_id}: tool call {call_id} arguments must be an object")
    ...
    for call_id, call in calls_by_id.items():
        ...
        if output.get("name") != call.get("name"):
            raise DatasetValidationError(f"{sample_id}: tool name mismatch for {call_id}")
        payload = output.get("output")
        if not isinstance(payload, dict) or payload.get("success") is not True:
            raise DatasetValidationError(f"{sample_id}: tool output {call_id} must be successful")
...
def _validate_v8_dispatch_lifecycle(record: dict[str, Any], sample_id: str) -> None:
    ...
    expected_names = (
        ["search_pipeformer_registry"] * registry_count
        + ["run_pipeformer_forecast"] * 3
        + ["set_decision_policy"]
    )
    forecast_arguments = [
    ...
    candidate_ids = [arguments.get("candidate_id") for arguments in forecast_arguments]
    context_fields = (
        "case_id",
    ...
    contexts = {
        tuple(arguments.get(key) for key in context_fields)
    ...
    disturbance_variable = forecast_arguments[0].get("disturbance_variable") if forecast_arguments else None
    ...
    *,
    ...
    for message_index, message in enumerate(messages):
        if not isinstance(message, dict):
            raise DatasetValidationError(f"{example_id}: message {message_index} must be an object")
        role = message.get("role")
        if role not in {"system", "user", "assistant", "tool_call", "tool_response"}:
            raise DatasetValidationError(
                f"{example_id}: unsupported message role {role!r}"
            )
        ...
        if role in {"system", "tool_call"} and "windows-first" in content.casefold():
            raise DatasetValidationError(
        ...
        if role == "tool_call":
            ...
            if isinstance(parsed_call, dict):
                ...
                if isinstance(arguments, dict) and is_host_absolute_path(
                    ...
                    raise DatasetValidationError(
                        f"{example_id}: tool_call cwd must be workspace-relative "
                        "or omitted"
                    )
    ...
    if requires_tools:
        ...
        if not schema_names <= registered_tool_names:
            ...
            raise DatasetValidationError(f"{example_id}: unregistered tool schema {unknown}")
    ...
    if projection == "trace_level" and messages[-1].get("role") != "assistant":
        raise DatasetValidationError(f"{example_id}: trace must end with a final assistant answer")
    ...
    if task_type == "tool_planning" and call_count == 0:
        raise DatasetValidationError(f"{example_id}: tool_planning must contain at least one tool_call")
    ...
    if projection != "constraint_multitask" or task_type != "tool_planning":
        if not assistant_messages or not assistant_messages[-1]["content"].strip():
            raise DatasetValidationError(f"{example_id}: final assistant response is required")
    ...
    if task_type in {
        ...
        if not isinstance(structured_target, dict):
            raise DatasetValidationError(f"{example_id}: structured assistant target must be an object")
    tool_calls = (item for item in messages if item.get("role") == "tool_call")
    for message in (*assistant_messages, *tool_calls):
        if "<think>" in message["content"].casefold():
            raise DatasetValidationError(f"{example_id}: hidden chain-of-thought targets are forbidden")
...
def _validate_tools(record: dict[str, Any], example_id: str) -> set[str]:
    ...
    if "windows-first" in raw_tools.casefold():
        raise DatasetValidationError(f"{example_id}: tool schema contains host-specific path policy")
    ...
    for schema in tools:
        try:
            ...
            raise DatasetValidationError(f"{example_id}: invalid function tool schema") from exc
        if not isinstance(name, str) or not name:
    ...
    *,
    ...
    while index < len(messages):
        ...
        if role == "tool_response":
            raise DatasetValidationError(f"{example_id}: tool_response must follow a tool_call")
        if role != "tool_call":
            index += 1
            continue
        call_count += 1
        ...
        if not isinstance(call, dict):
            raise DatasetValidationError(f"{example_id}: tool_call content must be an object")
        name = call.get("name")
        if name not in registered_tool_names:
            raise DatasetValidationError(f"{example_id}: unregistered tool {name!r}")
        if not isinstance(call.get("arguments"), dict):
            raise DatasetValidationError(f"{example_id}: tool_call arguments must be an object")
        ...
        if response.get("loss") is not False:
            raise DatasetValidationError(f"{example_id}: tool_response must not receive loss")
        payload = _parse_json(response["content"], f"{example_id}: tool_response")
        if not isinstance(payload, dict) or payload.get("success") is not True:
            if not allow_failed_responses:
                raise DatasetValidationError(f"{example_id}: tool_response must be successful")
        index += 2
...
def _validate_python_tool_sequence(
    calls: Sequence[dict[str, Any]], location: str
) -> None:
    ...
    for call in calls:
        ...
        if name == "write_file":
            ...
            try:
                ...
                raise DatasetValidationError(
                    f"{location}: write_file for {path!r} must contain complete "
                    "valid Python"
                ) from exc
    ...
    *,
    ...
    for record in records:
        ...
        if projection in _SOURCE_COVERAGE_PROJECTIONS:
            # These projections must keep both the prompt and the answer.
    ...
    if projection in _SOURCE_COVERAGE_PROJECTIONS:
        ...
        if answer_sources != sources.keys():
            raise DatasetValidationError(f"{projection}/{split}: answer_generation coverage mismatch")
